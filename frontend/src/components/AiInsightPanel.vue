  ...
  >
    ...
    <div v-else-if="error" class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs font-semibold text-amber-400">AI 경영 분석 일시 중단</p>
        <p class="mt-1 text-xs text-slate-400">{{ error }}</p>
        <p class="mt-1 text-xs text-slate-500">AI 분석은 부가 기능입니다. 아래 차트와 KPI는 정상 동작합니다.</p>
        <button
          @click="emit('retry')"
          class="mt-3 text-xs bg-blue-900/40 hover:bg-blue-800/60 text-blue-300 border border-blue-800 hover:border-blue-600 px-3 py-1.5 rounded-lg transition-colors"
        >
          다시 분석 요청
        </button>
      </div>
    ...
    <!-- ── 상태 3: AI 미설정 / 데이터 없음 ───────────────────────────────── -->
    <div v-else-if="!insight" class="flex items-center gap-3 text-slate-500">
    ...
    <div v-else>
      ...
      <!-- 추천 액션 -->
      <div v-if="insight.recommendation" class="flex items-start gap-2 rounded-lg bg-blue-900/30 px-3 py-2.5">
