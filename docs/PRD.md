...
| 테스트 | 단위 테스트 + E2E + CI/CD 파이프라인 | 74개 테스트 (단위 59 + E2E 15), 6-Job CI |

---

## 4. 기능 요구사항 (MoSCoW 우선순위 분류)

> **MoSCoW 기법**으로 기능별 우선순위를 분류합니다.
> - **Must Have (M):** 해커톤 MVP에 반드시 포함 — 없으면 제품 가치 없음
> - **Should Have (S):** 사용자 경험에 중요하나 MVP 없이도 핵심 가치 전달 가능
> - **Could Have (C):** 있으면 좋지만 시간 부족 시 제외 가능
> - **Won't Have (W):** 이번 해커톤 범위 외 — 향후 확장 백로그

### 4-1. Must Have — 핵심 기능 (MVP 필수)

| ID | 기능 | MoSCoW | 설명 | 구현 상태 |
|----|------|--------|------|----------|
| F-01 | KPI 카드 4종 | **M** | 이번 달 총 매출, 조제 건수, 방문 환자, 발주 지출 + 전월 대비 변화율 | 완료 |
| F-02 | 월별 매출 추이 | **M** | 바+라인 복합 차트, 기간 필터(3/6/12개월) | 완료 |
| F-03 | 의약품 유형별 매출 | **M** | ETC vs OTC 도넛 차트 | 완료 |
| F-04 | 환자 연령대 분포 | **M** | 8개 연령대별 도넛 차트 | 완료 |
| F-05 | 처방 기관 분석 | **M** | 의료기관별 처방전 유입 건수 수평 바 차트 — 병원 의존도 파악 핵심 | 완료 |
| F-06 | 도매상 지출 현황 | **M** | 도매상별 누적 지출 바 차트 | 완료 |
| F-07 | 급여/비급여 비율 | **M** | 건강보험 적용 여부별 지출 도넛 차트 | 완료 |
| F-08 | AI 경영 분석 | **M** | Gemini API로 경영 요약 + 하이라이트 + 경고 + 추천 액션 자동 생성 | 완료 |

### 4-2. Should Have — 부가 기능 (사용성 강화)

| ID | 기능 | MoSCoW | 설명 | 구현 상태 |
|----|------|--------|------|----------|
| F-09 | CSV 내보내기 | **S** | 6개 섹션 전체 CSV 다운로드, BOM UTF-8 (Excel 한글 호환) | 완료 |
| F-10 | 반응형 레이아웃 | **S** | 모바일~PC Tailwind breakpoint 자동 적응 (약사 모바일 접근 요구 반영) | 완료 |
| F-11 | Keep-Alive | **S** | Render 무료 플랜 15분 슬립 방지 10분 주기 핑 | 완료 |
| F-12 | 기간 필터 | **S** | 월별 매출 차트 3/6/12개월 클라이언트 슬라이싱 + 터치 스와이프 | 완료 |
| F-17 | 차트 드릴다운 | **S** | 월별 매출 바 클릭 → 상세(총매출/조제건수/건당평균) 패널 표시 | 완료 |
| F-18 | 로딩 진행률 | **S** | 7단계 진행률 바 + ARIA progressbar 접근성 | 완료 |

### 4-3. Could Have — 선택 기능 (시간 허용 시)

| ID | 기능 | MoSCoW | 설명 | 구현 상태 |
|----|------|--------|------|----------|
| F-19 | 에러 유형별 안내 | **C** | NETWORK/API/PARSE 유형별 사용자 친화적 메시지 + 자동 재시도 | 완료 |
| F-20 | 스켈레톤 로딩 | **C** | KPI 카드 4개 + 차트 6개 각각 형태별 스켈레톤 UI | 완료 |
| F-21 | Vue Transition | **C** | fade/slide-fade/toast 3종 트랜지션 애니메이션 | 완료 |

### 4-4. Won't Have — 향후 확장 (백로그)

| ID | 기능 | MoSCoW | 설명 |
|----|------|--------|------|
| F-13 | 다중 약국 지원 | **W** | 약국별 데이터 분리, 사용자 인증 → Pinia 전역 상태 관리 도입 시점 |
| F-14 | 알림 기능 | **W** | 매출 급감, 재고 부족 시 이메일/SMS 알림 |
| F-15 | 데이터 임포트 | **W** | 청구 프로그램 CSV 파일 자동 파싱·연동 |
| F-16 | 월간 리포트 PDF | **W** | AI 분석 결과를 PDF로 자동 생성·이메일 발송 |
